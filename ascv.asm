...
main:
   ...
   xor di, di; mov di, 0
   mov ah, 1
   int 21h; 输入1个字符如'a'->AL
   mov input, al
   xor bx, bx; bx=0
   mov bl, al; bl='a'
   mov cl, 4
   shl bx, cl; bx=bx<<4相当于bx='a'*16
             ; bx是'a'的点阵在asc数组中的偏移
   mov cx, 16; 画16行
next_row:
   mov al, asc[bx]; AL=每行的8个点,1位对应1点
   push cx
   push di
   mov cx, 8; 画8个点
next_dot:
   shl al, 1; al=al<<1,移出去的最高位->CF
   jnc is_blank
is_dot:
   mov dl, 4; 红点
   jmp show_dot
...
show_dot:
   ...
   pop di
   pop cx
   add bx, 1; bx->点阵的下一个字节即下面8个点
   add di, 320; di=屏幕下一行行首的地址
   sub cx, 1; 行数-1
   jnz next_row
   mov ah, 1
   int 21h; 敲任意键
