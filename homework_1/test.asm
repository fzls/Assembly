...
main:
    ...
    xor bx, bx
    mov ah, 1
    int 21h;输入第一个数字字符，如3F 的'3'
    cmp al, '9'
    jbe firstIsdigit
    cmp al, 'F'
    jbe firstIsCapChar
    cmp al, 'f'
    jbe firstIsUncapChar

firstIsdigit:
    sub al, 48
    mov bl, al ;bl = 3
    jmp firstInputFinished
firstIsCapChar:
    sub al, 55
    mov bl, al
    jmp firstInputFinished
...
firstInputFinished:
    mov cl, 4
    shl bx, cl;bx = 0030h, bl = 30h

    int 21h;输入第二个数字字符，如3F 的'F'
    cmp al, '9'
    jbe secondIsdigit
    cmp al, 'F'
    jbe secondIsCapChar
    cmp al, 'f'
    jbe secondIsUncapChar

secondIsdigit:
    sub al, 48
    add bl, al
    jmp secondInputFinished
secondIsCapChar:
    sub al, 55
    add bl, al;bl = 30h + 'F'-55 = 30h + 15 = 3Fh
    jmp secondInputFinished
secondIsUncapChar:
    sub al, 87
    add bl, al
    jmp secondInputFinished
secondInputFinished:
;now bl contains the input ex: 3Fh
    mov ax, 0013h
    int 10h

    mov cl, 4
    shl bx, cl; bx=bx<<4相当于bx='?'*16
             ; bx是'?'的点阵在asc数组中偏移
    mov cx, 16
next_row:
    mov al, asc[bx]
